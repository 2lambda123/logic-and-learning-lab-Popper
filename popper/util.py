...
def load_kbpath(kbpath):
    if len(kbpath) == 1:
        return fix_path(kbpath[0], "bk.pl"), fix_path(kbpath[0], "exs.pl"), fix_path(kbpath[0], "bias.pl")
    if len(kbpath) == 3:
        return fix_path(kbpath[0]), fix_path(kbpath[2]), fix_path(kbpath[1])
    
def fix_path(kbpath, filename=''):
    full_filename = os.path.join(kbpath, filename) if filename else kbpath
